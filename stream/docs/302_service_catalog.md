...
### `POST /api/catalog/v1/internal/catalog/sync/:short_id`
- **目的**: ~~Metadata Serviceがデータ更新を行った直後にコールされる。~~ 手動での再同期や、特定コンテンツの即時反映が必要な場合にコールする。
- **役割**:
  1. 対象データの最新版をMetadata Serviceから取得(DB直接参照)し、MongoDBのドキュメントを再構築・Upsert(上書き保存)する。
  2. (必要であれば検索インデックスの再生成などをキックする)

> [!NOTE]
> 通常のデータ同期は **Benthos** によるポーリングによって自動的に行われるため、正規の更新フローで本 API を呼び出す必要はない。
