...
当初はローカルの `lm-server`（Ollama / `qwen3.6-27b:iq3`）を第一候補としていたが、次の理由で `agy` CLI を主経路に置く。

- **台本の品質がこの機能の価値を直接決める**。要約の正確さと日本語の読みやすさが落ちれば、動画も記事も見る意味がなくなる。iq3 量子化の 27B で日本語の長文台本を書かせる品質は未知数であり、ここを賭けの対象にしたくない
- **認証は `GEMINI_API_KEY` を Secret 経由で渡す**。コンテナ実行環境で容易に取り回せる
- **1 日 1 回・数千字の生成**であり、コストが問題になる規模ではない

`lm-server` の Ollama は**捨てずに残す**。プロバイダをインタフェースで切り、環境変数で選択できるようにしておけば、外部に出したくない情報源を扱う場合や、`agy` が使えない状況のフォールバックとして使える。
...
**本番は `agy` CLI を使う**ことに決めた（§2.2）。台本の品質がこの機能の価値を直接決めるため、そこを未知数に賭けない判断である。ローカルの Ollama はプロバイダ切替の片側として残し、選別段のコスト削減やフォールバックに充てる。
