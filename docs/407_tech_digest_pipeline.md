...
**本番は `claude` CLI（モデル: Sonnet 5 / `claude-sonnet-5`）を採る。**

当初はローカルの `lm-server`（Ollama / `qwen3.6-27b:iq3`）を第一候補としていたが、次の理由で `claude` CLI を主経路に置く。

- **台本の品質がこの機能の価値を直接決める**。要約の正確さと日本語の読みやすさが落ちれば、動画も記事も見る意味がなくなる。iq3 量子化の 27B で日本語の長文台本を書かせる品質は未知数であり、ここを賭けの対象にしたくない
- **nuage-autopilot に `claude` CLI の運用実績がある**（`internal/runner`）。認証・実行・出力の取り回しが既知であり、新しい運用要素が増えない
- **1 日 1 回・数千字の生成**であり、コストが問題になる規模ではない
- Sonnet 5 を選ぶのは、この用途（選別と要約・構成）に Opus 相当の能力が要らないため。速度とコストの割に日本語品質が十分である

`lm-server` の Ollama は**捨てずに残す**。プロバイダをインタフェースで切り、環境変数で選択できるようにしておけば、外部に出したくない情報源を扱う場合や、`claude` が使えない状況のフォールバックとして使える。
...
2. **台本化（compose）**: 選ばれた記事から構造化台本を書く。品質が効くのはここ。`claude` CLI / Sonnet 5 を使う

出力は構造化 JSON に固定する（後述）。`claude -p` に台本スキーマを渡して JSON を書かせ、`batch-tech-feed` 側の `script.py` で検証してから後段へ流す。**バッチが受け取る境界は常に検証済みの台本であり、LLM の出力をそのままレンダリングに渡さない。**
...
│                  └─ lm-server (Ollama) or Claude API      │
...
| compose | `claude` CLI (Sonnet 5) | 1〜数分（台本数千字） |
...
**本番は `claude` CLI（Sonnet 5）を使う**ことに決めた（§2.2）。台本の品質がこの機能の価値を直接決めるため、そこを未知数に賭けない判断である。ローカルの Ollama はプロバイダ切替の片側として残し、選別段のコスト削減やフォールバックに充てる。
