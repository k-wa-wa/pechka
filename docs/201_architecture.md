...
スキーマ定義は `k8s/base/infra/postgres/migrations/*.up.sql` を参照。


| テーブル | 役割 |
| :--- | :--- |
| `discs` | Bluray ディスク管理（ラベル・名前） |
| `contents` | コンテンツ（動画・画像・VR・ドキュメント）の Single Source of Truth |
| `video_variants` | ABR 各品質バリアント（master / 1080p / 720p / 480p / audio） |
| `assets` | サムネイル・ポスター等の S3 キー |
